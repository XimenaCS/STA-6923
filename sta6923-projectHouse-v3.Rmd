...
The pie charts provide an overview of the distribution of each categorical variable in the dataset. Overall, the categories appear relatively well balanced, with no extreme skews. `Bedrooms`, `Bathrooms`, and `Floors` show moderate variation across their levels, while `Garage` availability is split almost 50/50 between “Yes” and “No.
...
house_v3 <- house_v2 %>%
  mutate(
    Area_per_Room = Area / (as.numeric(as.character(Bedrooms)) + 
                 as.numeric(as.character(Bathrooms)))
  )
summary(house_v3)
```
...
model_results <- model_results %>%
  arrange(RMSE)   # lower RMSE is better
...
-   The weak relationships between Price and the predictors, observed both in the exploratory analysis and in the models’ low R² values, suggest that the dataset may be simulated.
